...
export class GameState {
  ...
  tick(dt: number) {
    ...
    // SOFTLOCK KİLİDİ (#1082 cicektekstil25@gmail.com: "iki pompam da arızalı, tamir
    // parası yok, itibar düşüyor" — oyun ilerletilemez hâle geliyor). Sistem aynı anda
    // 2 arıza veriyordu; 2 pompalı oyuncuda bu İKİSİNİN DE bozulması demek: gelir sıfır,
    // tamir parası kazanma yolu yok, çıkmaz sokak. Artık SON ÇALIŞAN ÜNİTE asla bozulmaz.
    // Denge değişmiyor (arıza olasılığı, ceza, tamir bedeli aynı) — yalnız çıkmaz kapanıyor.
    const calisanPompa = this.pumps - this.brokenPumps.size
    const calisanSarj = this.evChargers - this.brokenChargers.size
    const sonUnite = (calisanPompa + calisanSarj) <= 1
    if (brokenCount < 2 && !sonUnite) {
