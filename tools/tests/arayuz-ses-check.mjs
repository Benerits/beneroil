...
// MERGE (30 Ağu): arka planda oyun worker ile akmaya devam ediyor (main'in kararı), ama
// görünmezken kart konumu güncellemenin anlamı yok — kontrol document.hidden ile korundu.
bekle(/if \(!document\.hidden && selectedBuilding && ui\.buildingCardVisible\)/.test(ana),
  'kamera kaydıkça kart yapıyla birlikte kayıyor (yalnız sekme görünürken)')
