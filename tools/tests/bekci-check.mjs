...
{
  katiCisim([])
  // KUYRUK OLUŞSUN (kapı açık): pompası kapanan araç konveyör kuralının GERÇEK
  // kapsamına girer — arkasından gelen kuyruk onu meşru olarak tutar. Bu senaryo
  // uydurma bir bayrak değil, oyunun kendi frenidir.
  ...
  let frenKare = 0, frenBaslangic = -1, muafAt = -1, kurtAt = -1, frendeKurtarma = false
  S.sur(900, { herKare: (i) => {
    const t = (i + 1) * DT
    const frende = A.blokT > 0 && !A.blokMuaf     // kural ŞU AN tutuyor
    if (frende) { frenKare++; if (frenBaslangic < 0) frenBaslangic = t }
