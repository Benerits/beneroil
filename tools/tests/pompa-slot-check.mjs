...
const olay = await import('../../src/trafik-olay.ts')
{
  ...
  // 3d''') iç içe YÖNLÜ ölçüm: yönü bilinen çiftte ayak izi (2.66 × 1.2) esas alınır
  {
    ...
    sur(3)
    check('iç içe: aynı şeritte boyuna 2.0 (< 2.15) olay', h.length === 1 && h[0].govde.k === 'icice', JSON.stringify(h.map(x => x.govde.k)))
    const h2 = kur(() => [yonlu(1, 0, 0, 1, 0), yonlu(2, 2.2, 0.1, 1, 0)])
    sur(3)
    check('iç içe: aynı şeritte boyuna 2.2 (BLOK_DUR tam duruş aralığı) olay DEĞİL', h2.length === 0, JSON.stringify(h2.map(x => x.govde.k)))
